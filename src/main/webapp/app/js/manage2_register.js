...
//			hompage: { url : "정상적인 URL이 아닙니다" },
    	...
    	submitHandler : function(form){
			var frm = $("#frm")[0];
			frm.action = "/app/manage2/manage2Add.do";
			frm.submit();
        }
...
//    	return this.optional(element) ||  /^.*(?=.*\d)(?=.*[a-zA-Z])(?=.*[!@#$%^&+=]).*$/.test(value);
	...
	getOption('#bank_code', 'bank', 'N', '');
	
	$("#corp_regist_num2").hide();
	
	$("#corp_type").change(function(){
	    var corp_type =  $(this).val();
	    
	    if(corp_type == "B"){
	    	$("#corp_regist_num2").show();
	    }else{
	    	$("#corp_regist_num2").hide();
	    }
	})
