...
public class MailSendService {
    ...
    public String sendAuthMail(String email) {
		...
		sb.append("	    <title>gmgpayment</title>	");
		sb.append("	</head>	");
		...
		sb.append("					회원이 아니신 경우나 메일 수신거부 회원이신데도 메일을 받으신다면 <a href='mailto:"+propertiesService.getString("email.address")+"' style='text-decoration:none; color:#333; '>"+propertiesService.getString("email.address")+"</a> 으로 연락해 주세요. ");
		sb.append("				</p>	");
		sb.append("			</section>	");
		sb.append("		</div>	");
		sb.append("			 ");	
        ...
        try {
            ...
            sendMail.setSubject(propertiesService.getString("email.title")+" 이메일 인증");
    ...
    public String sendAuthMailPW(String email, String pw) {
    	...
    	try {
    		MailUtils sendMail = new MailUtils(mailSender);
    		sendMail.setSubject(propertiesService.getString("email.title")+" 임시비밀번호");
    		sendMail.setText(sb.toString());
    		sendMail.setFrom(propertiesService.getString("email.address"), "관리자");
    		sendMail.setTo(email);
    		sendMail.send();
