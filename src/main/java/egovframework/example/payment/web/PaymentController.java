...
public class PaymentController {
	...
	@RequestMapping(value = "/payment/payment.do", method = {RequestMethod.POST, RequestMethod.GET})
	public String payment(Model model) throws IOException {
		return "payment/payment";
	}
	
	@RequestMapping(value = "/payment/payment_sugido.do", method = {RequestMethod.POST, RequestMethod.GET})
	public String payment_sugido(PaymentDefaultVO searchPaymentVO, Model model) throws IOException {
		return "payment/payment_sugido";
	}
	
	@RequestMapping(value = "/payment/payment_cancel.do", method = {RequestMethod.POST, RequestMethod.GET})
	public String payment_cancel(Model model) throws IOException {
		return "payment/payment_cancel";
	}
	
	@RequestMapping(value = "/payment/payment_api_result.do", method = {RequestMethod.POST, RequestMethod.GET})
	public String payment_api_terminal(HttpServletRequest request, PaymentDefaultVO searchPaymentVO, Model model) throws IOException, SQLException {
		
		searchPaymentVO.setPaymethod(request.getParameter("PAYMETHOD"));
		searchPaymentVO.setCpid(request.getParameter("CPID"));
		searchPaymentVO.setDaoutrx(request.getParameter("DAOUTRX"));
		searchPaymentVO.setAmount(request.getParameter("AMOUNT"));
		
		if(searchPaymentVO.getPaymethod().equals("CARDOFF")) {
			searchPaymentVO.setSettdate(request.getParameter("SETTDATE"));
			searchPaymentVO.setAuthno(request.getParameter("AUTHNO"));
			searchPaymentVO.setTip(request.getParameter("TIP"));
			searchPaymentVO.setTax(request.getParameter("TAX"));
			searchPaymentVO.setTerminalid(request.getParameter("TERMINALID"));
			searchPaymentVO.setAgentno(request.getParameter("AGENTNO"));
			searchPaymentVO.setCardtype(request.getParameter("CARDTYPE"));
			searchPaymentVO.setAllotmon(request.getParameter("ALLOTMON"));
			searchPaymentVO.setCardcode(request.getParameter("CARDCODE"));
			searchPaymentVO.setBuycode(request.getParameter("BUYCODE"));
			searchPaymentVO.setCardno(request.getParameter("CARDNO"));
			searchPaymentVO.setPaymenttype("3");
			
			paymentService.cardTerminal(searchPaymentVO);
			
		}else if(searchPaymentVO.getPaymethod().equals("CARDOFFCANCEL")) {
			
			searchPaymentVO.setCanceltype("0");
			searchPaymentVO.setCancelmemo("단말기 거래취소");
			searchPaymentVO.setCanceldate(request.getParameter("CANCELDATE"));
			
			paymentService.cardTerminalCancel(searchPaymentVO);
		}
		
		
		return "payment/payment_api_result";
	}
	
	@RequestMapping(value = "/payment/payment_api_card.do", produces="text/plain;charset=UTF-8", method = {RequestMethod.POST, RequestMethod.GET})
	public String payment_api_card(HttpServletRequest request, PaymentDefaultVO searchPaymentVO, Model model) throws IOException, SQLException {
		
		
		String paymethod = request.getParameter("PAYMETHOD");
		logger.debug("PAYMETHOD: "+paymethod);
		
		searchPaymentVO.setPaymethod(paymethod);
		searchPaymentVO.setCpid(request.getParameter("CPID"));
		searchPaymentVO.setDaoutrx(request.getParameter("DAOUTRX"));
		searchPaymentVO.setAmount(request.getParameter("AMOUNT"));
		
		if(searchPaymentVO.getPaymethod().equals("CARD")) {
			SimpleDateFormat format = new SimpleDateFormat ( "yyyyMMddHHmmss");
			Calendar time = Calendar.getInstance();
			String format_time = format.format(time.getTime());			
			
			searchPaymentVO.setSettdate(format_time);
			searchPaymentVO.setAuthno(request.getParameter("AUTHNO"));
			searchPaymentVO.setOrderno(request.getParameter("ORDERNO"));
			searchPaymentVO.setCardcode(request.getParameter("CARDCODE"));
			searchPaymentVO.setCardno(request.getParameter("CARDNO"));
			searchPaymentVO.setEmail(request.getParameter("EMAIL"));
			searchPaymentVO.setUserid(request.getParameter("USERID"));
			searchPaymentVO.setUsername(request.getParameter("USERNAME"));
			searchPaymentVO.setProductcode(request.getParameter("PRODUCTCODE"));
			searchPaymentVO.setPaymenttype("1");
			
			paymentService.shopCard(searchPaymentVO);
			
		}else if(searchPaymentVO.getPaymethod().equals("CARD_CANCEL")) {
			
			searchPaymentVO.setCanceltype("1");
			searchPaymentVO.setCancelmemo("카드 거래취소");
			searchPaymentVO.setCanceldate(request.getParameter("CANCELDATE"));
	...
	@RequestMapping(value = "/payment/cardSugido.do", produces="application/json;charset=UTF-8", method = {RequestMethod.POST, RequestMethod.GET})
	public @ResponseBody Map cardSugido(HttpServletRequest request, HttpServletResponse response, PaymentDefaultVO searchPaymentVO, Model model) throws IOException, SQLException, Exception {
...
//            result = "[결제 실패] 에러코드 : "+RESULTCODE+" 에러메세지 : "+ERRORMESSAGE;
	...
	@RequestMapping(value = "/payment/payment_link_sms.do", method = {RequestMethod.POST, RequestMethod.GET})
	public @ResponseBody Object payment_link_sms(PaymentDefaultVO searchPaymentVO, HttpServletRequest request, HttpServletResponse response, Model model) throws IOException, SQLException {
...
//		}
		...
		result = smsSendService.sendAuthSmsLink(searchPaymentVO.getUserphone(), cardUrlSend);
		
        return result;
	}
	
	@RequestMapping(value = "/payment/cardCancel.do", produces="application/json;charset=UTF-8", method = {RequestMethod.POST, RequestMethod.GET})
	public @ResponseBody Map cardCancel(HttpServletRequest request, HttpServletResponse response, PaymentDefaultVO searchPaymentVO, Model model) throws IOException, SQLException {
