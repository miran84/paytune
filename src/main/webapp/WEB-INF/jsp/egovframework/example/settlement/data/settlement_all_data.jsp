...
//	$("#pageUnit").val("${pageUnit}").prop("selected", true);
	...
	$("#dataCnt").text('총 <c:out value="${settlementVO.total_approval_cnt}" />개 / 결과 금액 합계 :<c:out value="${settlementVO.total_approval}" />원');
	
	$("#amount_sum").text('<c:out value="${settlementVO.total_approval}" />원');
	$("#total_cnt").text('<c:out value="${settlementVO.total_approval_cnt}" />건');
				...
				<table>
                    ...
                    <thead>
                        <tr>
                            ...
                            >터미널ID</button></th>
                            <th class="imei"><button type="button" id="btn_imei" onclick="frmOrder('imei');"
    							<c:if test="${searchSettlementVO.order_id eq 'imei' and searchSettlementVO.order_no eq '1'}">class="up"</c:if>
    							<c:if test="${searchSettlementVO.order_id eq 'imei' and searchSettlementVO.order_no eq '2'}">class="down"</c:if>   
                            >단말기번호</button></th>
                            ...
                            <th class="vat"><button type="button" id="btn_commission" onclick="frmOrder('commission');"
    							...
    							<c:if test="${searchSettlementVO.order_id eq 'commission' and searchSettlementVO.order_no eq '2'}">class="down"</c:if>                            
                            >가맹점 수수료</button></th>
                            <th class="all_vat"><button type="button" id="btn_commission2" onclick="frmOrder('commission2');"
                            ...
                            >수수료부가세</button></th>
                            <th class="pay"><button type="button" id="btn_amount2" onclick="frmOrder('amount2');"
    							<c:if test="${searchSettlementVO.order_id eq 'amount2' and searchSettlementVO.order_no eq '1'}">class="up"</c:if>
    							<c:if test="${searchSettlementVO.order_id eq 'amount2' and searchSettlementVO.order_no eq '2'}">class="down"</c:if>                            
                            >지급액</button></th>
						...
						<c:choose>
							<c:when test="${!empty resultList}">							
								...
								<tr>
									...
									<td onclick="frmView('<c:out value="${result.no}" />','<c:out value="${result.daoutrx}" />','<c:out value="${result.cpid}" />')" class="id"><span><c:out value="${result.userid}" /></span></td>
									<td onclick="frmView('<c:out value="${result.no}" />','<c:out value="${result.daoutrx}" />','<c:out value="${result.cpid}" />')" class="id"><span><c:out value="${result.cpid}" /></span></td>
                                    ...
                                    <td onclick="frmView('<c:out value="${result.no}" />','<c:out value="${result.daoutrx}" />','<c:out value="${result.cpid}" />')" ><span><c:out value="${result.imei}" /></span></td>
									<td onclick="frmView('<c:out value="${result.no}" />','<c:out value="${result.daoutrx}" />','<c:out value="${result.cpid}" />')" class="buy">매입</td>
                                    ...
                                    <td onclick="frmView('<c:out value="${result.no}" />','<c:out value="${result.daoutrx}" />','<c:out value="${result.cpid}" />')" class="price"><span><c:out value="${result.amount}" /></span></td>
									...
									<td onclick="frmView('<c:out value="${result.no}" />','<c:out value="${result.daoutrx}" />','<c:out value="${result.cpid}" />')" class="vat"><span><c:out value="${result.franchisee_commission}" /></span></td>
									<td onclick="frmView('<c:out value="${result.no}" />','<c:out value="${result.daoutrx}" />','<c:out value="${result.cpid}" />')" class="all_vat"><span><c:out value="${result.commission2}" /></span></td>
									<td onclick="frmView('<c:out value="${result.no}" />','<c:out value="${result.daoutrx}" />','<c:out value="${result.cpid}" />')" class="tax"><span><c:out value="${result.vat}" /></span></td>
									<td onclick="frmView('<c:out value="${result.no}" />','<c:out value="${result.daoutrx}" />','<c:out value="${result.cpid}" />')" class="pay"><span><c:out value="${result.amount2}" /></span></td>
                                    <td><button type="button" class="detail_btn pop_detail_btn" onclick="detail('<c:out value="${result.period}" />','<c:out value="${result.cpid}" />');">상세보기</button></td>
								    <c:choose>
									    <c:when test="${sessionScope.role_id eq '1001'}">
